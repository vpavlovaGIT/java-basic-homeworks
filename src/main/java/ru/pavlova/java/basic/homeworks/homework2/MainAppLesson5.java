...
public class MainAppLesson5 {
    public static void main(String[] args) {
        ...
        int[] arr2 = {2, 2, 2, 2};
        int[] arr3 = {3, 3, 3, 3, 3};
        int[] result = sumArrays(arr, arr2, arr3);
        System.out.println("Результат:\n" + "  " + Arrays.toString(arr));
        System.out.println("+ " + Arrays.toString(arr2));
        System.out.println("+ " + Arrays.toString(arr3));
        System.out.println("= " + Arrays.toString(result) + "\n");

        boolean result2 = sumOfParts(arr2);
        System.out.println(result2 + "\n");

        int[] arrDesc = {5, 1, -8, -10, 73};
        decreaseArray(arrDesc);

        int[] arrReverse = {1, 2, 3, 4};
        int[] reversedArray = reverseArray(arrReverse);
     ...
     * = {4, 5, 4, 1, 1}
     */
    private static int[] sumArrays(int[] arr, int[] arr2, int[] arr3) {
        int maxSize = Math.max(Math.max(arr.length, arr2.length), arr3.length);
     ...
     */
    private static boolean sumOfParts(int[] arr) {
        ...
        String output = Arrays.toString(strArr).replace("|,", "|");
     ...
     */
    private static boolean decreaseArray(int[] arrDesc) {
     ...
     */
    private static int[] reverseArray(int[] arrReverse) {
        ...
        while (left < right) {
            ...
            arrReverse2[left] = arrReverse2[right];
